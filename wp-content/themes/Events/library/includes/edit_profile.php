...
if(!$current_user->ID)
{
	wp_redirect(home_url().'/index.php?page=login');
	exit;
}
global $wpdb;
...
if($_POST)
{
	...
	}else
	{
		...
		$custom_metaboxes = templ_get_user_meta();
		...
		wp_redirect(home_url().'/?page=profile');
		exit;
	}
}
...
<div id="wrapper" class="clearfix">
     <div id="content" class="clearfix" >
        ...
        <div class="breadcrumb clearfix">
            <div class="breadcrumb_in"><?php yoast_breadcrumb('',' &raquo; '.EDIT_PROFILE_TITLE);  ?></div>
        </div>
    <?php } ?>
...
<form name="profileform" id="registerform" action="<?php echo get_option( 'home' ).'/?page=profile'; ?>" method="post" enctype="multipart/form-data">
<?php
		global $form_fields_usermeta;
        $validation_info = array(); 
        foreach($form_fields_usermeta as $key=>$val)
        { 
		if($val['on_profile'] && $key != 'login_type'){
        $str = ''; $fval = '';
        $field_val = $key.'_val';
        ...
        if($val['type']=='upload')
        {
			global $current_user;
			if($current_user->ID == '')
	            $str = '<input name="'.$key.'" id="'.$key.'" type="file" '.$val['extra'].' '.$uclass.' value="'.$fval.'" /> ';
			else
			{
				 $str = '<input name="'.$key.'"  type="file" '.$val['extra'].' '.$uclass.' value="'.$fval.'" /> ';
				  $str .= '<input  id="'.$key.'" type="hidden" '.$val['extra'].' '.$uclass.' value="'.$fval.'" /> ';
			}
            if($val['is_require'])
            {
                $str .= '<span id="'.$key.'_error"></span>';	
			...
			$str .= '<img src="'.$curauth->$key.'" height="70px" width = "70px"/> ';
        }
        else
        if($val['type']=='radio')
        {
        ...
        }else
        {
		 ...
		 if($val['type']=='multicheckbox' || $val['type']=='radio'){ $class = 'option_cls';}
        echo $val['outer_st'].$label.'<div class="form_cat_right '.$class.'">'.$val['tag_st'].$str.'</div>'.$val['tag_end'].$val['outer_end'];
        }
		}
?>
<input type="submit" name="registernow" value="<?php echo EDIT_BUTTON;?>" class="b_registernow" />
   
   <input type="button" name="Cancel" value="Cancel" class="b_registernow" onclick="window.location.href='<?php echo get_author_link($echo = false, $current_user->ID);?>'" />

</form>
</div>
<?php include_once(TT_REGISTRATION_FOLDER_PATH . 'registration_validation.php');?>
<h1 class="title_green"><span><?php echo CHANGE_PW_TEXT; ?></span></h1>
<p class="note"> <span class="required">*</span> <?php echo INDICATES_MANDATORY_FIELDS_TEXT;?></p>
<div class="clearfix"></div>

<div class="registration_form_box" id="chg_pwd">
        <form name="chngpwdform" id="chngpwdform" action="<?php echo get_option( 'home' ).'/?page=profile&amp;chagepw=1'; ?>" method="post">
        <?php if($message1) { ?>
          ...
          <?php } ?>
                ...
                <input type="password" name="cnew_passwd" id="cnew_passwd"  class="textfield" />
                 ...
                 <input type="submit" name="Update" value="<?php echo EDIT_BUTTON; ?>" class="b_registernow" onclick="return chk_form_pw();" />
                 <input type="button" name="Cancel" value="Cancel" class="b_registernow" onclick="window.location.href='<?php echo get_author_link($echo = false, $current_user->ID);?>'" />
                 <div class="clearfix"></div>
        </form>
    </div>
...
{
	if( document.forms["chngpwdform"]["new_passwd"].value == '')
	{
		alert("<?php echo NEW_PW_LONG_TEXT; ?>");
		document.getElementById('new_passwd').focus();
		return false;
	}
	if( document.forms["chngpwdform"]["new_passwd"].value.length < 4 )
	{
		alert("<?php echo NEW_PW_ALERT_TEXT; ?>");
		document.getElementById('new_passwd').focus();
		return false;
	}
	if( document.forms["chngpwdform"]["cnew_passwd"].value == '')
	{
		alert("<?php echo CONFIRM_NEW_PW_LONG_TEXT; ?>");
		document.getElementById('cnew_passwd').focus();
		return false;
	}
	if( document.forms["chngpwdform"]["cnew_passwd"].value.length < 4 )
	{
		alert("<?php echo CONFIRM_NEW_PW_ALERT_TEXT;?>");
		document.getElementById('cnew_passwd').focus();
		return false;
	}
	if( document.forms["chngpwdform"]["new_passwd"].value != document.forms["chngpwdform"]["cnew_passwd"].value)
	{
		alert("<?php echo CONFIRM_NEW_SAME_TEXT; ?>");
		document.getElementById('cnew_passwd').focus();
		return false;
	}
}
