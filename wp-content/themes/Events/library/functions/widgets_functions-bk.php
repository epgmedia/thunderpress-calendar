		...
		<ul class="category_list_view" id="widget_index_past_events_id" style="display:none;">
            <?php
			 ...
			 if($category)
					...
					$today = date('Y-m-d');
					$sqlsql .= " AND p.ID in (select $wpdb->postmeta.post_id from $wpdb->postmeta where $wpdb->postmeta.meta_key='end_date' and date_format($wpdb->postmeta.meta_value,'%Y-%m-%d')<'".$today."') ";
					$orderbysql = " (select $wpdb->postmeta.meta_value from $wpdb->postmeta where $wpdb->postmeta.meta_key='st_date' and $wpdb->postmeta.post_id=p.ID and $wpdb->postmeta.meta_key='featured_h' ) desc, AND (p.ID in ( select $wpdb->postmeta.post_id from $wpdb->postmeta where $wpdb->postmeta.meta_key='event_type' and $wpdb->postmeta.meta_value ='Regular event') )";
