...
if(@$_POST['priceact'] == 'addprice')
{
	...
	if($id)	{
		$wpdb->query("update $price_db_table_name set price_title=\"$price_title\", price_desc=\"$price_desc\",price_post_type='event',is_show='1', package_cost=\"$package_cost\",validity=\"$validity\",validity_per=\"$validity_per\",is_recurring=\"$is_recurring\",billing_num=\"$billing_num\",billing_per=\"$billing_per\",billing_cycle=\"$billing_cycle\",status=\"$status\",is_featured=\"$is_featured\",feature_amount=\"$feature_amount\",feature_cat_amount=\"$feature_cat_amount\" where pid=\"$id\"");
		$msgtype = 'edit_price';
	}else	{
		$insertprice = "insert into $price_db_table_name (price_title,price_desc,price_post_type,price_post_cat,is_show,package_cost,validity,validity_per,is_recurring,billing_num,billing_per,billing_cycle,status,is_featured,feature_amount,feature_cat_amount) values ('".$price_title."','".$price_desc."','event','".$price_post_cat."','1','".$package_cost."','".$validity."','".$validity_per."','".$is_recurring."','".$billing_num."','".$billing_per."','".$billing_cycle."','".$status."','".$is_featured."','".$feature_amount."','".$feature_cat_amount."')";
...
<a href="<?php echo home_url();?>/wp-admin/admin.php?page=manage_settings#option_display_price" name="btnviewlisting" class="l_back" title="<?php _e('Back to manage price list','templatic');?>"/><?php echo PRICE_BACK_LABLE; ?></a>
</h4>
