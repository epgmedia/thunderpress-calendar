...
<form action="<?php echo home_url();?>/wp-admin/admin.php?page=manage_settings&mod=user_meta&act=addedit#option_display_custom_usermeta" method="post" name="custom_fields_frm" onsubmit="return chk_userfield_form();">
	...
	$custom_msg = 'Here you can edit custom user meta detail.'; }else { _e('Add a field for users&rsquo; profile','templatic'); $custom_msg = 'Create a new field to show in user dashboard / profile section.';}?>
     ...
     <a href="<?php echo home_url();?>/wp-admin/admin.php?page=manage_settings#option_display_custom_usermeta" name="btnviewlisting" class="l_back" title="<?php _e('Back to Manage User information Fields List','templatic');?>"/><?php _e('&laquo; Back to Manage User information Fields List','templatic'); ?></a> 
 ...
 <div class="option option-select" >
  ...
  <div class="option option-select"  >
    ...
    <div class="section">
      <div class="element">
           <input type="text" name="htmlvar_name" id="htmlvar_name" value="<?php echo $post_val->htmlvar_name;?>" size="50" <?php if(isset($_REQUEST['cf']) && $_REQUEST['cf']!=''){?>  readonly="readonly"<?php }?> />
     </div>
  ...
  <?php if($post_val->htmlvar_name != 'user_email' && $post_val->htmlvar_name != 'user_fname'){ ?>
	 <div class="option option-select"  >
  ...
  <div class="option option-select" id="show_on_listing_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
    ...
    <div class="section">
      ...
      <div class="description"><?php _e('Specify whether you want to show this field in &quot;Registration Form&quot;.','templatic');?></div>
    </div>
  </div> <!-- #end -->
    <?php //} ?>
  <div class="option option-select" id="show_on_detail_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
    ...
    <div class="section"> 
      ...
      <div class="description"><?php _e('Specify whether you want to display this field on &quot;User&rsquo;s Profile page&quot;.','templatic');?></div>
    </div>
  </div> <!-- #end -->
