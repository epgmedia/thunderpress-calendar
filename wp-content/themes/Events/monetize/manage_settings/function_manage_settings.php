...
function get_post_custom_fields_templ($post_types,$category_id='',$show_on_page = '',$is_search = '') {
	...
	$post_query .=  " order by sort_order ASC";
...
function get_post_custom_fields_admin_templ($post_types,$category_id='',$show_on_page = '',$is_search = '') {
	global $wpdb,$custom_post_meta_db_table_name;
	$post_query = "select * from $custom_post_meta_db_table_name where is_active=1";
	if(!strstr($post_types,',')){
		if($is_search != ''){
			$post_query .= " and (post_type = '".$post_types."' or post_type='organizer' or post_type='both')";
		} else {
			$post_query .= " and (post_type = '".$post_types."' or post_type='organizer' or post_type='both')";
		}
		...
		if($is_search != ''){
			$post_query .= " and (post_type IN ($post_types) or post_type='organizer' or post_type='both')";
		} else {
			$post_query .= " and (post_type IN ($post_types) or post_type='organizer' or post_type='both')";
		}
	}
