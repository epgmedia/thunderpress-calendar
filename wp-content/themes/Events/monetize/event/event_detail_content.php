...
else
{
	...
	<div class="tabber">
	 <ul class="tab">
		<li id="li_location_map" <?php if( $gallery_map_flag == 'Map & Gallery Both - Default Map'){ echo 'class="active"'; }?>> <a href="javascript:void(0):" onclick="show_hide_tabs('map');"><?php _e('Location Map','templatic');?></a></li>
		<li id="li_image_gallery" <?php if( $gallery_map_flag == 'Map & Gallery Both - Default Gallery') { echo 'class="active"'; }?> > <a href="javascript:void(0):" onclick="show_hide_tabs('gallery');"><?php _e('Image Gallery','templatic');?></a></li>
		<?php if (get_option('ptthemes_print') == 'Yes' ) { ?>
		<li class="fr" > <a href="#" onclick="window.print();return false;" class="i_print"><?php _e('Print','templatic');?></a></li>
		<?php } ?>
	 </ul>
	</div>
	<?php }
