	...
	case 'post_category' :
			...
			if (is_array($templ_events)) {
				foreach($templ_events as $key => $templ_event) {
					$edit_link = home_url()."/wp-admin/edit.php?".CUSTOM_CATEGORY_TYPE1."=".$templ_event->slug."&post_type=".CUSTOM_POST_TYPE1;
		...
		case 'post_tags' :
			...
			if (is_array($templ_event_tags)) {
				foreach($templ_event_tags as $key => $templ_event_tag) {
					$edit_link = home_url()."/wp-admin/edit.php?".CUSTOM_TAG_TYPE1."=".$templ_event_tag->slug."&post_type=".CUSTOM_POST_TYPE1;
