...
<div class="container-02">
	<div class="graybox">    
    <form name="registerform" id="registerform" action="<?php echo get_option( 'home' ).'/?page=login&amp;action=register'; ?>" method="post" enctype="multipart/form-data" >  
      <input type="hidden" name="reg_redirect_link" value="<?php echo $_SERVER['HTTP_REFERER'];?>" />
      <?php do_action('templ_registration_form_start');?>
		<?php
		/* CODE TO DISPLAY CUSTOM FIELDS ON REGISTRATION FORM */
		...
		<?php	/* CODE TO ADD PLAYTHRU */
			$play_options = get_option('ayah_options');
			if(!isset($play_options['enable_register_form']))
			{
				$play = get_option('ptthemes_captcha_option');
				$play_opt = get_option('ptthemes_recaptcha');
				$option = explode(",",$play_opt);
				if(file_exists(ABSPATH.'wp-content/plugins/are-you-a-human/areyouahuman.php') && is_plugin_active('are-you-a-human/areyouahuman.php')  && $play == 'PlayThru' && in_array("User Registration Page",$option))
				{
...
<?php } else {?> <div class="new_user_reg"><?php echo 'New user registration is disabled';?></div><?php } ?>