...
}else
{
	...
	$daily_event = $post_meta['daily_event'][0];
...
if(file_exists(ABSPATH.'wp-content/plugins/are-you-a-human/areyouahuman.php') && is_plugin_active('are-you-a-human/areyouahuman.php') && $play == 'PlayThru'  && in_array('Submit Event Page',$option) && $_REQUEST['pid'] == '')
{
	...
	if(!$score)
	{
		wp_redirect(site_url().'/?ptype=post_event&backandedit=1&invalid=playthru');
		exit;
	}
}
?>
    ...
    <?php } ?>
      ...
      <div class="event_information">      	
        <div class="date_info">
         ...
         <?php
	 if($st_date){?>
         <span><?php _e('Start Date');?> :</span> <?php echo get_formated_date($st_date);?> <br />
         <?php }?>
         <?php if($end_date){?>
        <span><?php _e('End Date');?>  :</span> <?php echo get_formated_date($end_date);?>  <br />
        <?php }?>
        <?php if($st_time && $end_time){?><span><?php _e('Time');?>  : </span> <?php echo get_formated_time($st_time)?> <?php _e('to');?> <?php echo get_formated_time($end_time)?><?php }?> </p>
		 <p> 
          <?php if($phone){ ?>
         <span><?php _e('Phone');?> :</span> <?php echo $phone;?>  <br /> 
         <?php }?>  
         <?php if($email){ ?>           
		<span><?php _e('Email');?> :</span> <?php echo $email;?></p>	
         <?php }?>
          </div> 
          ...
          <div class="location">
          	<?php if($address){ ?>
            <p> 
            <span><?php _e('Location');?>  : </span>   <br />
           <?php echo $address;?></p>
            <?php }?>
            <?php if($website){ ?>
            <p><a href="<?php echo $website;?>"><?php _e('Website');?></a></p>
             <?php }?>
          </div>
      </div>
...
function show_hide_tabs(type)
{
	...
	}else if(type=='gallery')
	{
       ...
       <div class="tabber">
   		 <ul class="tab">
            <li id="li_location_map" class="active"> <a href="javascript:void(0):" onclick="show_hide_tabs('map');"><?php _e('Location Map');?></a></li>
            <li id="li_image_gallery" class="" > <a href="javascript:void(0):" onclick="show_hide_tabs('gallery');"><?php _e('Image Gallery');?></a></li>
         </ul>
       </div>
...
<h3><?php _e('Organized by');?></h3>
<img src="<?php echo $organizer_logo; ?>" style="height:105px;width:105px;" alt="" class="organized_logo"  title=""  />
        	<div class="organized_content" >
              <p> <?php _e('Organized by');?>  <a href="<?php echo get_author_posts_url($post->post_author);?>"><?php echo $author_info->display_name;?></a></p> 
...
<?php _e('Tel');?>: <?php echo $organizer_contact;?> <br />
<?php }?>
<?php if($organizer_mobile){?>
<?php _e('Mobile');?>: <?php echo $organizer_mobile;?><br />
<?php }?>
<?php if($organizer_email  && get_option('ptthemes_email_on_detailpage')=='Yes'){?>
<?php _e('Email');?>: <?php echo $organizer_email;?><br />
<?php }?>
<?php if($organizer_website){?>
<?php _e('Website');?>: <a href=" <?php echo $organizer_website;?>"><?php echo $organizer_website;?></a>
<?php }?>
</p> 
<a class="b_contact" href="#" ><?php _e('Contact the Organizer');?></a>
</div>
<div class="basicinfo">
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
			<?php	
