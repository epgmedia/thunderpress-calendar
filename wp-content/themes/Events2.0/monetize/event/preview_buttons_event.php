...
{
	$form_action_url = get_option( 'siteurl' ).'/?ptype=paynow_event';
}else
{
	$form_action_url = get_ssl_normal_url(get_option( 'siteurl' ).'/?ptype=paynow_event');
}
?>
...
?>
	...
	<input type="button" name="Delete" value="<?php echo PRO_DELETE_BUTTON;?>" class="fr b_delete" onclick="window.location.href='<?php echo get_option('siteurl');?>/?ptype=delete&pid=<?php echo $_REQUEST['pid']?>'" />
    <?php 
	global $current_user;
	if($current_user->ID)
	{
		$cancle_url = get_author_posts_url($echo = false, $current_user->ID);	
	}else
	{
		$cancle_url = site_url();
	}
...
<a href="<?php echo get_option('siteurl');?>/?ptype=post_event&backandedit=1<?php if($_REQUEST['pid']){ echo '&pid='.$_REQUEST['pid'];}?><?php if($_REQUEST['renew']){echo '&renew=1';}?>" class="b_goback fl" ><?php echo PRO_BACK_AND_EDIT_TEXT;?></a>

<?php 
global $current_user;
if($current_user->data->ID)
{
	$cancle_url = get_author_posts_url($echo = false, $current_user->data->ID);	
}else
{
	$cancle_url = site_url();
}
