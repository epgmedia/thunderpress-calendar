<?php
/*
NAME : CATEGORY DISPLAY FILE
DESCRIPTION : THIS FILE WILL BE CALLED ON SUBMIT EVENT PAGE. THE CODE MENTIONED IN THIS FILE WILL DISPLAY THE CATEGORIES ON SUBMIT EVENT PAGE. CATEGORIES WILL BE DISPLAYED IN SELECT BOX OR IN CHECK BOX  AS PER USER SELECTION IN BACK END BASIC SETTINGS.
*/
global $wpdb;
...
if($catinfo)
{
	/* CODE TO DISPLAY THE CATEGORIES IN CHECKBOX */
	...
	foreach($catinfo as $catinfo_obj)
	{
		...
		/* END OF CODE */
		/* CODE TO DISPLAY CATEGORIES IN SELECT BOX */
		...
		$cp = $catprice->term_price; 
			if((isset($_REQUEST['category']) && $_REQUEST['category'] != '') || (isset($_SESSION['theme_info']['category']) && $_SESSION['theme_info']['category'] != '') ) { 
				if($_REQUEST['category'] !=""){
				$cat_term = explode(',',$_REQUEST['category']); }else{
				$cat_term = explode(',',$_SESSION['theme_info']['category']);
				}
				...
				<?php }
					...
					<option <?php if(isset($_SESSION['theme_info']['category']) && $_SESSION['theme_info']['category'] == $term_tax_id){echo 'selected="selected"'; }?> value="<?php if($cp != ""){ echo $termid.",".$term_tax_id.",".$catprice->term_price; }else{ echo $termid.",".$term_tax_id.","."0"; }?>"><?php if($cp != ""){ echo $name." (".display_amount_with_currency($cp).")"; }else{ echo $name."(".display_amount_with_currency('0').") "; } ?></option>
					<?php
				 $child = get_term_children( $term_tax_id ,CUSTOM_CATEGORY_TYPE1);
				 foreach($child as $child_of)
				 { 
 		...
 		$currency = fetch_currency(get_option('currencysym'),'currency_symbol');
		$position = fetch_currency(get_option('currencysym'),'symbol_position');
		$amount =0;
		if($position == '1'){
