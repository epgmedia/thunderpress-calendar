...
{
	wp_redirect(get_option( 'siteurl' ).'/?ptype=preview&msg=nopaymethod');
	exit;
}
global $current_user;
...
{	
	if($_POST['Submit_and_Pay'])
	{
		...
		$custom = array("address" 		=> $property_info['address'],
						...
						"organizer_logo"		=> $property_info['organizer_logo'],
		...
		/** ---- Save the variables of recurring event BOF -----**/
			...
			$custom['daily_event'] = $_SESSION['theme_info']['daily_event'];	
		...
		{	global $wpdb;
			$termtable = $wpdb->prefix."term_relationships";
				...
				if(is_array($post_category)) {
							 ...
							 if($_REQUEST['pid'])
							  {
								if($property_info['renew'])
								  {
									$cat_cntcat = $wpdb->query("UPDATE " . $termtable . " set term_taxonomy_id = '".$mycat[0]."' where object_id = '".$post_id."' and term_order = '0'");
								  }
								  else
								  {
					...
					if($_REQUEST['pid']){
						if($property_info['renew']) {	
							$cat_cntcat = $wpdb->query("UPDATE " . $termtable . " set term_taxonomy_id = '".$property_info['category']."' where object_id = ".$post_id." and term_order = '0'");
						} else {
							$cat_cntcat = $wpdb->query("UPDATE " . $termtable . " set term_taxonomy_id = '".$property_info['category']."' where object_id = ".$post_id." and term_order = '0'");	
						}
					} else {
						$cat_cntcat = $wpdb->query("INSERT INTO " . $termtable . "(object_id, term_taxonomy_id, term_order)VALUES ('".$post_id."','".$property_info['category']."','0') ");
					}
				}
		}
