...
<div class="option option-select" <?php if(@$post_val->is_edit == '0'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
    ...
    <div class="section">
	  ...
	  ?>
                 <select name="post_type1" id="post_type">
				  <?php foreach ($custom_post_types as $content_type) {  ?>
                  	<option value="<?php echo $content_type->name; ?>" <?php if($post_val->post_type==$content_type->name){ echo 'selected=selected';}?>><?php echo $content_type->name;?></option>
                 <?php }?>
				<option value="organizer"<?php if($post_val->post_type=='organizer') { echo 'selected="selected"'; }else{ if(!$post_val->post_type){ echo 'selected=selected'; } } ?>><?php _e('organizer','templatic'); ?></option>
                  </select>
      	   </div>
