...
function display_custom_post_field($custom_metaboxes,$session_variable,$geo_latitude='',$geo_longitude='',$geo_address='',$post_type){
	...
	<?php
		...
		<label><?php echo $site_title.$is_required; ?></label>
		<textarea name="<?php echo $name;?>" id="<?php echo $name;?>" PLACEHOLDER="<?php echo  $val['default']; ?>" class="mce <?php if($style_class != '') { echo $style_class;}?>" <?php echo $extra_parameter;?> ><?php if(@$value != '') { echo $value; }else{ echo  stripslashes($val['default']); } ?></textarea>
		<?php
		...
		<?php
			...
			{  $chkcounter = 0;
				...
				for($i=0;$i<count($option_values_arr);$i++)
				{
					...
					if (trim(@$value) == trim($option_values_arr[$i])){ $seled='selected="selected"';}	
					echo '
		...
		<?php
		 ...
		 <input type="file" value="<?php echo @$value; ?>" name="<?php echo $name; ?>" style="font-size:28px; cursor:pointer;opacity:0;position:absolute; bottom:0; right:3px;"/>
		 </div>
	...
	<?php } 
					if($key == "event_type"){ 
						...
						if($_SESSION[$session_variable] && $_REQUEST['backandedit'])
						{
							...
							$daily_event = $_SESSION[$session_variable]['daily_event'];
                    ...
                    <div class="form_row clearfix" id="recurring_event" <?php if(trim(strtolower($event_type)) == trim(strtolower('Recurring event'))){  ?>style="display:block;" <?php }else{ ?> style="display:none;" <?php } ?>>
						...
						<div class="form_daily_event form_row clearfix">
							...
							<?php _e('Is this a daily event ?','templatic'); ?></label>
						</div>
