    ...
    <div class="author_photo">
			...
			<?php else : echo get_avatar($curauth->ID, 75 ); endif; ?>
			 <?php 
			  ...
			  {
			   ?>
              <div class="editProfile"><a href="<?php echo get_option('siteurl');?>/?page=profile" ><?php echo PROFILE_EDIT_TEXT;?> </a> </div>
              <?php } ?>
    </div>
			...
			<div class="agent_biodata">
						  ...
						  <?php if($curauth->user_url){?>
                           <?php 
							 ...
							 ?>
                          <span><a href="<?php echo $website; ?>"><?php echo PRO_WEBSITE_TEXT;?> </a></span><?php }?>						  
						  <?php 
							 ...
							 foreach($form_fields_usermeta as $key=> $_form_fields_usermeta)
							  {
								  ...
								  if($key != 'user_web' && $key != 'description' && $key != 'user_twitter' && $key != 'user_photo' &&  $key != 'user_fname' && $key != 'user_lname' && $key != 'user_email' ):
	...
	?>
     ...
     <?php if (function_exists('pagenavi')) { ?>
	  ...
	  <?php } else { 
		...
		?><a id="edit_fb_fields" style="float:right; clear:both;<?php if(get_usermeta($curauth->ID,'appid')){?> display:block; <?php } ?>" onclick="return showFacebookSetting('show_facebook_setting');">
			<?php echo SHOW_FACEBOOK_SETTING; ?>
		</a><?php } ?>
		<div id="show_api_fields" <?php if(get_usermeta($curauth->ID,'appid')){?> style="display:none;" <?php } ?>>
	  	<table>
			<tr>
				<td>
				...
				<td>
					<input type="text" name="appid" id="appid" value="<?php echo get_usermeta($curauth->ID,'appid'); ?>" />
				</td>
			</tr>
			<tr>
				<td>
				...
				<td>
					<input type="text" name="secret_id" id="secret_id" value="<?php echo get_usermeta($curauth->ID,'secret'); ?>" />
				</td>
			</tr>
			<tr>
				<td>
				...
				<td>
					<input type="text" name="page_id" id="page_id" value="<?php echo get_usermeta($curauth->ID,'pageID'); ?>" />
				</td>
			</tr>
			<tr>
				<td colspan="2">
		...
		<div id="hide_fb_fields" style="float:right; clear:both;<?php if(get_usermeta($curauth->ID,'appid')){?> display:none; <?php } else { ?>display:block; <?php } ?>" onclick="return showFacebookSetting('hide_facebook_setting');">
			<?php echo HIDE_FACEBOOK_SETTING; ?>
		</div>
		<div id="responsecontainer">
		  <?php $appID = get_usermeta($curauth->ID,'appid');
			if(_iscurlinstalled())
			{
				if($appID)
