...
// Custom comment loop
function custom_comment($comment, $args, $depth) {	global $post,$current_user;
       $GLOBALS['comment'] = $comment; ?>
	
<li class="comment wrap threaded" id="comment-<?php comment_ID() ?>">
    <div class="meta-left">
        <div class="meta-wrap">
		<?php if($current_user->user_photo != '') : ?>
			<img src="<?php echo $current_user->user_photo; ?>" width="75" height="75" />
			<?php else : ?><span class="gravatar_bg"> </span><?php echo get_avatar( $comment, 78, $template_path . ''.get_bloginfo('template_directory').'/images/gravatar.png' ); endif; ?>
        </div>
    </div>
