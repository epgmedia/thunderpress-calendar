...
//	$st_logo  = '<a href="'.esc_url( home_url( '/' ) ).'" title="'.esc_attr( get_bloginfo('name','display')).'">'.$logo.'</a>'. "\n";
	$st_logo  = '<a href="http://www.thunderpress.net" title="'.esc_attr( get_bloginfo('name','display')).'">'.$logo.'</a>'. "\n";
...
function add_to_attend_event($post_id,$st_date='',$end_date='')
{
	...
	update_usermeta($current_user->ID, 'user_attend_event', $user_meta_data);
	if($st_date)
	{
		...
		update_usermeta($current_user->ID, 'user_attend_event_st_date', $user_meta_start_date);
	}
	if($end_date)
	{
		...
		update_usermeta($current_user->ID, 'user_attend_event_end_date', $user_meta_end_date);
	}
	...
	if(!$st_date)
	{
		echo '<span class="span_msg">'.$current_user->display_name." ,".REMOVE_EVENT_MSG." ".$post->post_title." ?".'</span><a href="javascript:void(0);" class="addtofav b_review" onclick="javascript:addToAttendEvent(\''.$post_id.'\',\'remove\');">'.REMOVE_EVENT_TEXT.'</a>';exit;	
		}
	...
	{
		echo '<span class="span_msg">'.$current_user->display_name." ,".REMOVE_EVENT_MSG." ".$post->post_title." ?".'</span><a href="javascript:void(0);" class="addtofav b_review" onclick="javascript:addToAttendEvent(\''.$post_id.'\',\'remove\',\''.$st_date.'\',\''.$end_date.'\');">'.REMOVE_EVENT_TEXT.'</a>';exit;	
	}
}
...
function remove_from_attend_event($post_id,$st_date='',$end_date='')
{
	...
	update_usermeta($current_user->ID, 'user_attend_event_st_date', $user_attend_event_st_date);
	...
	update_usermeta($current_user->ID, 'user_attend_event_end_date', $user_attend_event_end_date);
	}
	if(!$st_date)
	{
		echo '<span class="span_msg">'.$current_user->display_name." ,".ATTEND_EVENT_MSG.'</span><a class="addtofav b_review" href="javascript:void(0);"  onclick="javascript:addToAttendEvent(\''.$post_id.'\',\'add\');">'.ATTEND_EVENT_TEXT.'</a>';exit;
	}
	else
	{
		echo '<span class="span_msg">'.$current_user->display_name." ,".ATTEND_EVENT_MSG.'</span><a class="addtofav b_review" href="javascript:void(0);"  onclick="javascript:addToAttendEvent(\''.$post_id.'\',\'add\',\''.$st_date.'\',\''.$end_date.'\');">'.ATTEND_EVENT_TEXT.'</a>';exit;
	}
...
function attend_event_html($user_id,$post_id)
{
	...
	<span class="span_msg"><?php echo $current_user->display_name.", ".REMOVE_EVENT_MSG; ?></span>
	...
	?>
	<span id="attend_event_<?php echo $post_id;?>" class="fav">
	<span class="span_msg"><?php echo $current_user->display_name.", ".ATTEND_EVENT_MSG." ".$post->post_title." ?"; ?></span>
...
function attend_event_html1($user_id,$post_id,$st_date,$end_date)
{
	...
	$a.="<span id='attend_event_$post_id-$st_date' class='fav'>
	<span class='span_msg'>".$current_user->display_name.", ".ATTEND_EVENT_MSG." ".$post->post_title." ?</span>
...
function event_get_days_names(){
	return array (1 => __ ( 'Mon' ), 2 => __ ( 'Tue' ), 3 => __ ( 'Wed' ), 4 => __ ( 'Thu' ), 5 => __ ( 'Fri' ), 6 => __ ( 'Sat' ), 7 => __ ( 'Sun' ) );
}
...
{
	 $appID = get_usermeta($user_id,'appid');
	 $secret = get_usermeta($user_id,'secret');
	 $pageID = get_usermeta($user_id,'pageID');
	$config = array(
		'appId' => $appID,
		'secret' => $secret,
	  );
