...
function author_filter_where($where)
{
	...
	$post_ids = get_usermeta($user_id,'user_attend_event');
	$final_ids = '';
	if($post_ids)
	  {
...
function searching_filter_where($where) {
	...
	if($saddress)
	{
		if (strlen($saddress) == 2) {
			$where .= " AND ($wpdb->postmeta.meta_key like 'address' and ($wpdb->postmeta.meta_value like \"% $saddress %\" or $wpdb->postmeta.meta_value like \"%, $saddress %\" or $wpdb->postmeta.meta_value like \"%,$saddress,%\" or $wpdb->postmeta.meta_value like \"%, $saddress,%\")) ";
		} else {
...
{      
	...
	{
		$orderby = "(select $wpdb->postmeta.meta_value from $wpdb->postmeta where $wpdb->postmeta.post_id=$wpdb->posts.ID and $wpdb->postmeta.meta_key = 'st_date') ASC";
	}
	return $orderby;
