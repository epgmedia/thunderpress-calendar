...
switch ($a->process()) {
   ...
   case 1:  // Successs
	...
	$redirectUrl = get_option('siteurl')."/?ptype=payment_success&pid=".$last_postid;
...
if($paymentFlag == 0)
{
	 wp_redirect(get_option('siteurl')."/?ptype=checkout");
	 exit;
}
...
//if(!$paymentFlag)
//{
	//wp_redirect(get_option('siteurl').'/?ptype=checkout&paymentmethod='. $_REQUEST['paymentmethod'].'&shippingmethod='.$_REQUEST['shippingmethod']);
	//exit;
//}
?>