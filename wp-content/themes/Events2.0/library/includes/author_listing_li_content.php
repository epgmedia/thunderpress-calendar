...
<div class="event_list <?php if(get_post_meta($post->ID,'featured_type',true) != 'none'){?>event_list_featured<?php }?>clearfix">
    ...
    <p class="date"> <span><?php _e('Start Date');?>: </span> <?php echo get_formated_date(get_post_meta($post->ID,'st_date',true));?><br />  
    ...
    <span class="author_link"> 
		...
		{
		?>
        <a href="<?php echo get_option('siteurl');?>/?ptype=post_event&pid=<?php echo $post->ID;?>"><?php echo EDIT_TEXT;?></a> | 
        <?php	
		}else
		{
		?>
        <a href="<?php echo get_option('siteurl');?>/?ptype=post_event&renew=1&pid=<?php echo $post->ID;?>"><?php echo RENEW_TEXT;?></a> |
		<?php
		}
		?>
        <a href="<?php echo get_option('siteurl');?>/?ptype=preview&pid=<?php echo $post->ID;?>"><?php echo DELETE_TEXT;?></a> </span>  
       <?php }?>
    </p>
	<?php }
							...
							<li>
								<div class='date_info'>
									<p>
										 ...
										 <?php _e('to');?> <?php echo $get_post_end_date[1]; ?> <br/>
										<span>Time  : </span> <?php  echo get_formated_time(get_post_meta($post->ID,'st_time',true))?> <?php _e('to');?> <?php echo get_formated_time(get_post_meta($post->ID,'end_time',true)); ?>
									</p>
								</div>
