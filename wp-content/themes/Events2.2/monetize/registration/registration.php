...
{
	wp_redirect(get_option( 'home' ).'/?page=myaccount');
	exit;
}
...
function retrieve_password() {
	...
	$message .= '<p>You can login to : <a href="'.get_option( 'home' ).'/?page=login' . "\">Login</a> or the URL is :  ".get_option( 'home' )."/?page=login</p>";
	$message .= '<p>Thank You,<br> '.get_option('blogname').'</p>';
	$user_email = $user_data->user_email;
	$user_name = $user_data->user_nicename;
	$fromEmail = get_site_emailId();
	$fromEmailName = get_site_emailName();
	$store_login = get_option('home').'/?page=login';
	...
	$search_array = array('[#$user_name#]','[#$user_login#]','[#$user_password#]','[#$site#]','[#$site_login_url#]','[#$site_login_url_link#]');
...
function register_new_user($user_login, $user_email) {
	...
	$option = get_option('ptthemes_captcha_option');
	if((in_array('User Registration Page',$pcd)) && is_plugin_active('wp-recaptcha/wp-recaptcha.php') && $option=='WP-reCaptcha'){
		...
		if (!$resp->is_valid ) {
			wp_redirect(home_url().'/?page=login&page1=sign_up&ecptcha=captch');
			exit;
		} 
	}
	/* END OF CODE */
	...
	$play_option = get_option('ptthemes_recaptcha');
	$option = explode(",",$play_opt);	
	if(in_array("User Registration Page",$option) && is_plugin_active('are-you-a-human/areyouahuman.php') && strtolower(get_option('ptthemes_captcha_option')) == strtolower('PlayThru'))
	{	
		...
		if($score == '')
		{
			wp_redirect(home_url().'/?page=login&page1=sign_up');
			exit;	
		}
	}
	/* END OF CODE */
	...
	update_user_meta($user_id, 'first_name', $_POST['user_fname']); /* User Address Information Here */
	...
	if ( $user_id ) 
	{
		...
		$store_login = '<a href="'.get_option('home').'/?page=login">Click Login</a>';
		$store_login_link = get_option('home').'/?page=login';
...
if ( defined('RELOCATE') ) { /* Move flag is set */
	...
	if ( dirname($schema . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF']) != get_option('home') )
...
case 'logout' :
	...
	//$redirect_to = get_option( 'home' ).'/?page=login&loggedout=true';
...
case 'retrievepassword' :
	if ( $http_post ) {
		...
		if ( !is_wp_error($errors) ) {
			wp_redirect(home_url().'/?page=login&page1=sign_in&checkemail=confirm');
			exit();
		}else
		{
			wp_redirect(home_url().'/?page=login&page1=sign_in&emsg=fw');
			exit();
		}
	}
...
case 'rp' :
	...
	if ( ! is_wp_error($errors) ) {
		wp_redirect(home_url().'/?page=login&action=login&checkemail=newpass');
		exit();
	}

	wp_redirect(home_url().'/?page=login&action=lostpassword&page1=sign_in&error=invalidkey');
	exit();
...
case 'register' :
	...
	if ( $http_post ) {
		...
		if(is_wp_error($errors))
		{
			wp_redirect(home_url().'?page=login&page1=sign_up&emsg=1');
		}
		if ( !is_wp_error($errors) ) 
		{
...
default:
	...
	if(is_wp_error($user))
	{
		if(strstr($_SERVER['HTTP_REFERER'],'ptype=post_event') && $_POST['log']!='' && $_POST['pwd']!='')
		{
	...
	if(($_POST['log'] && $errors) || ($_POST['log']=='' && $_REQUEST['testcookie']))
	{
		...
		}else
		{
			if($_REQUEST['loginfrm'] == 1)
			  {	
				wp_redirect(home_url().'?page=login&emsg=2');
			  }
			else
			  {
				  wp_redirect(home_url().'?page=register&emsg=1');
			  }
		}
		exit;
	}
...
<?php get_header(); ?>
<div id="page">
<div id="wrapper" class="clearfix fullwidth" >
	<div id="content">
...
<ul class="page-nav"><li><?php if ( get_option( 'ptthemes_breadcrumbs' ) == 'Yes') { yoast_breadcrumb('',' &raquo; '.SIGN_IN_PAGE_TITLE); } ?></li></ul>
<div class="row">
<?php
