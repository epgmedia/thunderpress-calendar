...
<div id="top_strip">
	<div id="top_strip_in" class="top_strip_in clearfix">
		...
		<ul class="children">
			...
			<?php global $current_user;
				...
				<li><a href="<?php echo wp_logout_url(home_url()); ?>" title="Log out of this account"><?php echo LOGOUT_TEXT;?></a></li>
				<?php } else { ?>
			<li><a href="<?php echo home_url()."/?page=login"; ?>"><?php echo LOGIN_TEXT;?></a></li>
			<?php } ?>
		</ul>
		</div>
	<?php	} else {
 ...
 <div  id="header">
    ...
    <?php } ?>
     ...
     <a class="b_sbumit" href="<?php echo get_option('home'); ?>/?ptype=post_event"><?php _e('Submit Event','templatic');?> </a>
     <?php }?>
   ...
   <?php if ( get_option('ptthemes_show_menu') == 'No' )
		{  
			...
			wp_nav_menu( array( 'container_class' => 'menu-header clearfix', 'theme_location' => 'secondary','menu_class' => 'sub-menu', ));
			} else { ?>
				...
				<ul id="menu-below-main" class="sub-menu">
					<li class="hometab <?php if ( is_home() && @$_REQUEST['page']=='' && @$_REQUEST['ptype'] =='') { ?> current_page_item <?php } ?>"><a href="<?php echo home_url(); ?>/"><?php _e('Home','templatic'); ?></a></li>							
						...
						<li class="<?php if(@$_REQUEST['ptype']=='post_event' || @$slug =='post_event'){ echo 'current_page_item'; }?>">
							<a href="<?php echo get_option('home'); ?>/?ptype=post_event"><?php _e('Add Event','templatic'); ?></a>
							</li><?php } ?>
				</ul>
			<?php } ?>
