...
error_reporting(0);
	...
	if(isset($_POST['templatic_login']) && isset($_POST['templatic_username']) && $_POST['templatic_username']!=''  && isset($_POST['templatic_password']) && $_POST['templatic_password']!='' && @$_REQUEST['page'] == 'tmpl_theme_update')
	{ 
		$arg=array(
			'method' => 'POST',
			'timeout' => 45,
		...
		{		
			...
			if(in_array('Events - developer license',$product) || in_array('Events - standard license',$product))
			{
				$successfull_login=1;	
				
				$_SESSION['success_user_login'] = 'yes';

				$download_link=$data->product['Events - developer license'];
			}else
			{
				$warnning_message="We don't find Events in your templatic account, you will not be able to update without a license";
			}			
		}
	}else{
	...
	?>
		  ...
		  <?php if($flag ==1){ ?>
			  ...
			  <a class="button-secondary" href="http://templatic.com/members/mydownloads/Events/theme/Events.zip" target="blank"><?php _e('Download latest Version','templatic'); ?></a> 
			  
			  </div>
		  <?php } ?>
		   <div id='pblogo'>
               <img src="<?php echo esc_url( get_template_directory_uri()."/images/templatic.jpg"); ?>" style="margin-right: 50px;" />
			   <?php echo '<h3>'.get_current_theme().' Updates</h3>'; ?>
		   </div> 
           <?php
		...
		?>
		 <?php if($flag ==1){ 
			if(!isset($successfull_login) && @$successfull_login !=1 && !$session):?>
			   ...
			   <?php _e('Enter your templatic login credentials to update your Events theme to the latest version.',DOMAIN);?></p>
          ...
          <?php else:								
				 ...
				 echo ' Events - theme <a id="TB_closeWindowButton" href="'.$download.'" target="_parent" class="button-secondary">Update Now</a>';
			 endif;
			 } ?>
        </div>
<?php
