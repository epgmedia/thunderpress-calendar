...
from wp_postmeta 
where	meta_key = 'st_time' AND meta_value <> '' and meta_value <> '?'
order by post_id
");
   ...
   if (mysql_num_rows($result2)>0) {
	   ...
	   while ($row2 = mysql_fetch_array($result2)) {
		   ...
		   if(empty($meta_value2)) {
			   $sql = "update wp_postmeta set meta_value='24:00:00' where meta_id=$meta_id2";
		...
		$sql = "insert into wp_postmeta (post_id,meta_key,meta_value) values ('$post_id','end_time','24:00:00')";
		$result3 = mysql_query($sql);
		echo "$sql<hr>";
