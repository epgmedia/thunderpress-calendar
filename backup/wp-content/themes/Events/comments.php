...
<div id="respond">
    ...
    <?php if ( get_option('comment_registration') && !$user_ID ) : ?>
        <p class="mustlogin"><?php _e('You must be','templatic'); ?> <a href="<?php echo get_option('home'); ?>/wp-login.php?redirect_to=<?php echo urlencode(get_permalink()); ?>"><?php  _e('logged in','templatic'); ?></a> <?php _e('to post a comment.','templatic'); ?></p>
    <?php else : ?>	
        ...
        <?php } ?>
		<?php if ( $user_ID ) : ?>
                <p class="mustlogin"><?php  _e('logged in','templatic'); ?> &rarr; <a href="<?php echo get_option('home'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo wp_logout_url(); ?>" title="Log out of this account"><?php _e('Logout','templatic'); ?> &raquo;</a></p>
            <?php else : ?>
                <p class="commpadd clearfix">
                <label for="author" ><small class="author"><?php _e('Name','templatic'); ?> <?php  if ($req) _e('*','templatic'); ?></small></label>
                ...
                <p class="commpadd clearfix">
                <label for="email"><small class="email2"><?php _e('Mail','templatic'); ?> <?php  if ($req) _e('*','templatic'); ?></small></label>
...
function check_comment()
{
	...
	<?php if ($req){?>
	if(document.getElementById('author').value=='' || document.getElementById('author').value=='<?php _e('Name','templatic');?>')
	{
		alert('<?php _e('Please enter Name','templatic')?>');
		document.getElementById('author').focus();
		return false;
	}
	if(document.getElementById('email').value=='' || document.getElementById('email').value=='<?php _e('Email','templatic');?>')
	{
