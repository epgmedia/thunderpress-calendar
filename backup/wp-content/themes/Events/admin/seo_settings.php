<?php
/*
Name : templ_seo_add_custom_box_css
Description : Add css for SEO settings
*/
...
.inside p.row label  { width: 120px; float:left; margin-right:10px;  } 
.inside p.row input{ float:left;} 
.inside p.row .page_seo_desc, .inside p.row .page_seo_kw {  height:120px !important; }
.inside p.row .seo_button { margin-left:130px; }
</style>
<?php	
}
...
function templ_seo_add_custom_box() {
    ...
    add_meta_box( 'myplugin_sectionid', __( 'SEO Settings','templatic'),'myplugin_inner_custom_box', CUSTOM_POST_TYPE1 );
}
